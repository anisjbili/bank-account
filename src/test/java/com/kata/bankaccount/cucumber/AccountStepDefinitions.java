...
public class AccountStepDefinitions {
    ...
    public void balance_equal_to(int expectedBalance) {
        Assertions.assertEquals(new Balance(new BigDecimal(expectedBalance)),account.getBalance());
    }
    ...
    public void console_print_print_the_following(DataTable dataTable) {
        ...
        for (StatementLine statementLine : statementLines) {
            expectedOut +="| "+ statementLine.getOperation().getType() + " | " + LocalDateTime.now(fixedClock).format(formatter) + " | " + statementLine.getOperation().getAmount().getValue() + " | " + statementLine.getBalance().getValue() + " |\n";
        }
